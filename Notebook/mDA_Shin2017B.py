...
dataset1 = Shin2017B(accept=True)
...
epoch_path = "../epoch_data/Shin2017B"
...
for i, sub in enumerate(subjects):
    try:
        ...
        print("Please execute mDA_Shin2017B_preprocess.ipynb first.")
...
for scenario in cv_list.keys():
    for algo in overall_acc[scenario][align_methods[0]]:
        ...
        ax.set_yticklabels(['mRPA-{2}', 'mRPA-{3}', 'mRPA-{4}', 'mRPA-{5}', 'mRPA-{6}', 'mRPA-{7}', 'mRPA-{8}', 'RPA'], fontsize=12)
        ax.set_xlabel('Classification accuracy', fontsize=12)
        plt.grid()
        plt.savefig(f"Shin2017B_w_k_{scenario}_{algo}.pdf", bbox_inches='tight')
        plt.show()
